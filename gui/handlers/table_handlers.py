...
class TableActionHandler:
    ...
    def _perform_archive(self, account_names: List[str], category: str, archive_settings: Dict):
        ...
        try:
            ...
            # ВАЖНО: Обновляем данные после успешной архивации
            if results['success']:
                self._refresh_after_archive(category)
    ...
    def _refresh_after_archive(self, category: str):
        """Обновляет данные после архивации"""
        try:
            logger.info("🔄 Обновляем данные после архивации...")

            # Обновляем таблицу для текущей папки
            self._update_table_only(category)

            # Обновляем статистику в родительском компоненте
            self._update_parent_stats_only(category)

            logger.info("✅ Данные обновлены после архивации")

        except Exception as e:
            logger.error(f"❌ Ошибка обновления после архивации: {e}")
    ...
    def _show_archive_results(self, results: Dict, archive_settings: Dict):
        ...
        if results['success']:
            ...
            show_success(
                ...
                f"Аккаунтов заархивировано: {archived_count}\n"
                f"Аккаунты перемещены в архив и удалены из системы"
            )
        else:
            # Ошибка архивации
            show_error(
                "Ошибка архивации",
                f"❌ {results['message']}\n"
                f"Файлы аккаунтов восстановлены в исходное место"
            )
