...
async def async_main():
    ...
    # 3. ИСПРАВЛЕНО: Сначала инициализируем AccountManager
    try:
    ...
    # 4. ИСПРАВЛЕНО: Теперь создаем главное окно (когда данные уже загружены)
    main_win = MainWindow()
    main_win.show()
    logger.info("🖥️  Главное окно открыто")

    # 5. Показываем уведомление о готовности
    await asyncio.sleep(0.5)
    try:
        from gui.notifications import show_success
        show_success("Приложение готово", "desktop-Traffer успешно запущен!")
    except Exception as e:
