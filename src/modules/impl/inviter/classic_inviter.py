...
class ClassicInviterProcess(BaseInviterProcess):
    ...
    def _run_inviting(self):
        ...
        logger.info("[{self.profile_name}] 🚀 Запуск классического инвайтинга")
        ...
        if initial_accounts_to_request < accounts_needed:
            logger.warning(f"[{self.profile_name}] ⚠️ Недостаточно аккаунтов! Требуется: {accounts_needed}, доступно: {available_accounts}")
            logger.info(
        ...
        if not allocated_accounts:
            logger.error("[{self.profile_name}] ❌ Не удалось получить свободные аккаунты")
            return
            ...
            try:
                ...
                if account_index >= len(allocated_accounts):
                    ...
                    if additional_accounts:
                        ...
                        logger.info(f"[{self.profile_name}] ✅ Получено дополнительно {len(additional_accounts)} аккаунтов")
                    else:
                        # Возвращаем чат обратно в очередь
                        self.chat_queue.put(chat)
                        logger.warning(f"[{self.profile_name}] ⚠️ Нет аккаунтов для чата {chat}, отложен")
                        break
                ...
                if not chat_accounts:
                    ...
                    self.chat_queue.put(chat)
                    logger.warning(f"[{self.profile_name}] ⚠️ Не удалось выделить аккаунты для чата {chat}")
                    break
                ...
                logger.info(f"[{self.profile_name}] ✅ Запущен поток для чата #{chat_index}: {chat} (аккаунтов: {len(chat_accounts)})")
        ...
        if self.chat_queue.qsize() > 0:
            logger.warning(f"[{self.profile_name}] ⚠️ Остались необработанные чаты: {self.chat_queue.qsize()}")
...
class ChatWorkerThread(threading.Thread):
    ...
    async def _work(self):
        """Основная работа с чатом - ВЕРСИЯ С ПОТОКАМИ И ИСПРАВЛЕННЫМ LOOP"""
        logger.info(f"🚀 [{self.profile_name}]-[Чат-{self.chat_link}] Начинаем работу с {self.chat_link}")
        ...
        # ВАЖНО: Предварительно создаем клиенты в текущем event loop
        ...
        while not chat_completed and not self.parent.stop_flag.is_set():
            ...
            logger.info(f"🚀 [{self.profile_name}]-[Чат-{self.chat_link}] Запущено {len(worker_threads)} потоков воркеров")
            ...
            while not self.parent.stop_flag.is_set():
                ...
                if self.stop_all_workers:
                    logger.warning(
                        f"⚠️ [{self.profile_name}]-[Чат-{self.chat_link}] Остановка всех воркеров. Причина: {self.chat_stop_reason}")
                    chat_completed = True
                    break
                ...
                if self.parent.config.success_per_chat > 0 and self.chat_success >= self.parent.config.success_per_chat:
                    logger.success(f"✅ [{self.profile_name}]-[Чат-{self.chat_link}] Достигнут лимит успешных инвайтов: {self.chat_success}")
                    chat_completed = True
                    break
            ...
            logger.info(f"🔓 [{self.profile_name}]-[Чат-{self.chat_link}] Освобождено завершивших работу аккаунтов: {released_count}")
            ...
            if self.stop_all_workers:
                logger.warning(f"⚠️ [{self.profile_name}]-[Чат-{self.chat_link}] Чат остановлен. Причина: {self.chat_stop_reason}")
                break

            # Если есть пользователи и работа не завершена - запрашиваем новые аккаунты
            if not self.parent.user_queue.empty():
                logger.info(f"🔄 [{self.profile_name}]-[Чат-{self.chat_link}] Запрашиваем новые аккаунты для продолжения работы")
                ...
                if not new_accounts:
                    logger.warning(f"⚠️ [{self.profile_name}]-[Чат-{self.chat_link}] Нет свободных аккаунтов, завершаем работу с чатом")
                    chat_completed = True
                    break

                # ВАЖНО: Инициализируем клиенты для новых аккаунтов
                logger.info(f"🔧 [{self.profile_name}]-[Чат-{self.chat_link}] Инициализируем клиенты для новых аккаунтов...")
                ...
                logger.info(f"✅ [{self.profile_name}]-[Чат-{self.chat_link}] Получено новых аккаунтов: {len(new_accounts)}")
    ...
    def _run_worker_in_thread(self, worker_id: int, account_data, active_workers: list, lock: threading.Lock):
        """Обертка для запуска воркера в отдельном потоке - БЕЗ СОЗДАНИЯ НОВОГО LOOP"""
        # Добавляем себя в список активных
        with lock:
            active_workers.append(account_data.name)

        try:
            # НЕ создаем новый event loop!
            # Используем asyncio.run_coroutine_threadsafe для выполнения в loop ChatWorkerThread

            # Используем loop из ChatWorkerThread (не из главного потока!)
            ...
            try:
                ...
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}] Ошибка освобождения аккаунта {account_data.name}: {e}")
    ...
    async def _run_worker(self, worker_id: int, account_data):
        """Воркер для инвайтинга - УПРОЩЕННАЯ ВЕРСИЯ БЕЗ СОЗДАНИЯ КЛИЕНТА"""
        account_name = account_data.name
        account = account_data.account

        logger.info(f"👷 [Чат-{self.chat_link}]-[Поток-{worker_id}] Запуск с аккаунтом {account_name}")

        try:
            # Клиент уже должен быть создан в основном потоке
            if not account.client:
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Клиент не создан для {account_name}")
                return

            # Подключаемся
            if not await account.connect():
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Не удалось подключить {account_name}")
                return

            # Проверяем авторизацию
            if not await account.client.is_user_authorized():
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Аккаунт {account_name} не авторизован")
            ...
            try:
                ...
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Ошибка получения информации об аккаунте: {e}")
                await account.disconnect()
                return
            ...
            if join_result == "STOP_CHAT":
                logger.warning(
                    f"⚠️ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Остановка всех воркеров чата. Причина: {self.chat_stop_reason}")
                self.stop_all_workers = True
                await account.disconnect()
                return
            elif join_result == "FROZEN_ACCOUNT":
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Аккаунт заморожен, завершаем работу")
                await account.disconnect()
                return
            elif join_result != "SUCCESS":
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Не удалось присоединиться к чату")
                await account.disconnect()
                return
            ...
            while not self.parent.stop_flag.is_set() and not self.stop_all_workers:
                ...
                if self.parent.config.success_per_chat > 0:
                    if self.chat_success >= self.parent.config.success_per_chat:
                        logger.info(
                            f"🎯 [Чат-{self.profile_name}] Достигнут лимит успешных инвайтов для чата: {self.chat_success}")
                        break
                ...
                if self.parent.config.success_per_account > 0:
                    if invites_count >= self.parent.config.success_per_account:
                        logger.info(f"🎯 [{self.profile_name}]-[Поток-{worker_id}] Достигнут лимит инвайтов для аккаунта: {invites_count}")
                        break
            ...
            logger.info(f"📊 [{self.profile_name}]-[Поток-{worker_id}] Завершен. Инвайтов: {invites_count}, ошибок: {errors_count}")
    ...
    async def _join_chat(self, account, worker_id: int) -> bool:
        ...
        try:
            ...
            if result == "ALREADY_PARTICIPANT":
                logger.info(f"ℹ️ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Уже в чате {self.chat_link}")
                return "SUCCESS"
                ...
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Чат не найден: {self.chat_link}")
                return "STOP_CHAT"

            elif result == "REQUEST_SENT":
                logger.warning(f"⏳ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Отправлен запрос на вступление в {self.chat_link}")
                return False

            elif result == "FLOOD_WAIT":
                logger.warning(f"⏳ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Нужно подождать перед присоединением в {self.chat_link}")
                return False

            elif isinstance(result, str) and result.startswith("ERROR:"):
                logger.error(f"❌ [Поток-{worker_id}] Ошибка: {result}")
                return False

            else:
                # Успешно присоединились
                logger.info(f"✅ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Присоединился к чату {self.chat_link}")
            ...
            logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Ошибка присоединения к чату {self.chat_link}: {e}")
            return False
    ...
    async def _invite_user(self, user: InviteUser, account, account_name: str, worker_id: int) -> bool:
        ...
        logger.info(f"📨 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Пытаемся добавить @{username} в {self.chat_link}")
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"❓ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Пользователь @{username} не существует")
            ...
            # 1.5 НОВОЕ: Проверяем общие чаты если есть ID текущего чата
            if self.chat_telegram_id and old_common_chats > 0:

                try:
                    # Получаем список общих чатов
                    from telethon.tl.functions.messages import GetCommonChatsRequest
                    ...
                    for chat in common_chats_result.chats:
                        if hasattr(chat, 'id') and chat.id == self.chat_telegram_id:
                            logger.warning(f"👥 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} уже в чате! (Чат: {self.chat_link})")
                    ...
                    logger.debug(f"✅ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} не найден в текущем чате (Чат: {self.chat_link})")

                except Exception as e:
                    # Если не удалось проверить - продолжаем инвайт
                    logger.debug(f"⚠️ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Не удалось проверить общие чаты: {e}")
            ...
            if result.missing_invitees:
                logger.warning(f"🔒 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} - настройки приватности (Чат: {self.chat_link})")
            ...
            if new_common_chats <= old_common_chats:
                logger.warning(f"💸 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} добавлен и сразу списан (Чат: {self.chat_link})")
            ...
            # Успешно добавлен
            logger.success(f"✅ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} успешно добавлен! (Чат: {self.chat_link})")
            ...
            logger.error(f"🚫 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Спамблок при добавлении @{username} (Чат: {self.chat_link})")
            ...
            logger.warning(f"🚫 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} заблокирован в Telegram (Чат: {self.chat_link})")
            user.status = UserStatus.NOT_FOUND
            user.last_attempt = datetime.now()
            ...
            logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Недостаточно прав в чате (Чат: {self.chat_link})")
            user.status = UserStatus.ERROR
            user.last_attempt = datetime.now()
            ...
            logger.warning(f"📊 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} уже в максимальном количестве чатов (Чат: {self.chat_link})")
            user.status = UserStatus.ERROR
            user.last_attempt = datetime.now()
            ...
            if "CHAT_MEMBER_ADD_FAILED" in error_text:
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Не удалось добавить @{username} (Чат: {self.chat_link})")
                user.status = UserStatus.ERROR
                user.error_message = "Ошибка добавления"

            elif "You're banned from sending messages" in error_text:
                logger.error(f"🚫 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Аккаунт заблокирован для инвайтов (Чат: {self.chat_link})")
                user.status = UserStatus.ERROR
                user.error_message = "Аккаунт заблокирован"

            elif "user was kicked" in error_text.lower():
                logger.warning(f"🚫 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} был ранее кикнут из чата (Чат: {self.chat_link})")
                user.status = UserStatus.ALREADY_IN
                user.error_message = "Был кикнут"

            elif "already in too many channels" in error_text.lower():
                logger.warning(f"📊 [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] @{username} в слишком многих чатах (Чат: {self.chat_link})")
                user.status = UserStatus.ERROR
                user.error_message = "Слишком много чатов"

            else:
                logger.error(f"❌ [{self.profile_name}]-[Поток-{worker_id}]-[{account.name}] Неизвестная ошибка для @{username}: {e} (Чат: {self.chat_link})")
                user.status = UserStatus.ERROR
