...
class InviterProfileManager:
    ...
    def update_chats_database(self, profile_name: str, chats_list: List[str]) -> bool:
        ...
        try:
            logger.debug(f"🔍 [ДИАГНОСТИКА] Начинаем update_chats_database")
            logger.debug(f"   💬 Профиль: '{profile_name}'")
            logger.debug(f"   📥 Входящих данных: {len(chats_list)}")
            ...
            logger.debug(f"🔍 [ДИАГНОСТИКА] Папка профиля: {profile_folder}")
            logger.debug(f"   📁 Абсолютный путь: {profile_folder.resolve()}")
            logger.debug(f"   ✅ Папка существует: {profile_folder.exists()}")

            # Логируем первые несколько чатов для проверки
            if chats_list:
                sample_chats = chats_list[:5]  # Первые 5
                logger.debug(f"🔍 [ДИАГНОСТИКА] Образец входящих чатов: {sample_chats}")

            # НОВАЯ ВАЛИДАЦИЯ И ОЧИСТКА
            logger.debug(f"🔍 [ДИАГНОСТИКА] Запускаем валидацию чатов...")
            validated_chats, stats = self._validate_and_clean_chats(chats_list)

            # Детальная диагностика валидации
            logger.debug(f"🔍 [ДИАГНОСТИКА] Результат валидации чатов:")
            logger.debug(f"   📥 Входящих: {stats['input_count']}")
            logger.debug(f"   ✅ Валидных: {stats['valid_count']}")
            logger.debug(f"   🔄 Дублей удалено: {stats['duplicates_removed']}")
            logger.debug(f"   ❌ Невалидных: {stats['invalid_count']}")
            ...
            # САМАЯ ВАЖНАЯ ЧАСТЬ - СОХРАНЕНИЕ
            logger.debug(f"🔍 [ДИАГНОСТИКА] Начинаем сохранение чатов в файл...")
            ...
            logger.debug(f"✅ [ДИАГНОСТИКА] Данные чатов обновлены в памяти")

            logger.debug(f"🎉 [ДИАГНОСТИКА] Процесс завершен успешно: {len(validated_chats)} чатов")
            return True
    ...
    def _save_users_database_with_debug(self, profile_folder: Path, users_list: List[str]) -> bool:
        ...
        try:
            ...
            logger.debug(f"🔍 [ДИАГНОСТИКА] Детальная диагностика сохранения пользователей:")
            logger.debug(f"   📁 Папка профиля: {profile_folder}")
            logger.debug(f"   📁 Существует папка: {profile_folder.exists()}")
            logger.debug(f"   📄 Путь к файлу: {users_file}")
            logger.debug(f"   📊 Количество для записи: {len(users_list)}")
            ...
            try:
                ...
                logger.debug(f"✅ [ДИАГНОСТИКА] Права на запись в папку есть")
            except Exception as e:
            ...
            if users_list:
                content = '\n'.join(users_list)
                logger.debug(f"🔍 [ДИАГНОСТИКА] Длина контента для записи: {len(content)} символов")
                logger.debug(f"🔍 [ДИАГНОСТИКА] Первые 100 символов: '{content[:100]}'")
            else:
                content = ""
                logger.debug(f"🔍 [ДИАГНОСТИКА] Записываем пустой файл")

            # КЛЮЧЕВАЯ ОПЕРАЦИЯ - ЗАПИСЬ
            logger.debug(f"🔍 [ДИАГНОСТИКА] Выполняем запись в файл...")
            users_file.write_text(content, encoding='utf-8')
            logger.debug(f"✅ [ДИАГНОСТИКА] Запись выполнена без ошибок")

            # ПРОВЕРКА РЕЗУЛЬТАТА
            logger.debug(f"🔍 [ДИАГНОСТИКА] Проверяем результат записи...")
            ...
            logger.debug(f"✅ [ДИАГНОСТИКА] Файл создан, размер: {file_size} байт")
            ...
            try:
                ...
                logger.debug(f"✅ [ДИАГНОСТИКА] Проверка чтения:")
                logger.debug(f"   📊 Строк в файле: {len(saved_lines)}")
                logger.debug(f"   📊 Ожидалось строк: {len(users_list)}")
                ...
                if len(saved_lines) == len(users_list):
                    logger.debug(f"🎉 [ДИАГНОСТИКА] Количество записей совпадает!")
                    return True
                else:
    ...
    def _save_chats_database_with_debug(self, profile_folder: Path, chats_list: List[str]) -> bool:
        ...
        try:
            ...
            logger.debug(f"🔍 [ДИАГНОСТИКА] Детальная диагностика сохранения чатов:")
            logger.debug(f"   📁 Папка профиля: {profile_folder}")
            logger.debug(f"   📁 Существует папка: {profile_folder.exists()}")
            logger.debug(f"   📄 Путь к файлу: {chats_file}")
            logger.debug(f"   📊 Количество для записи: {len(chats_list)}")

            # ИСПРАВЛЕНО: Убеждаемся что папка точно существует
            if not profile_folder.exists():
                logger.debug(f"🔍 [ДИАГНОСТИКА] Создаем папку профиля...")
                profile_folder.mkdir(parents=True, exist_ok=True)

                if profile_folder.exists():
                    logger.debug(f"✅ [ДИАГНОСТИКА] Папка создана успешно")
                else:
                    logger.error(f"❌ [ДИАГНОСТИКА] Не удалось создать папку!")
                    return False
            ...
            # КЛЮЧЕВАЯ ОПЕРАЦИЯ - ЗАПИСЬ
            logger.debug(f"🔍 [ДИАГНОСТИКА] Выполняем запись в файл чатов...")
            chats_file.write_text(content, encoding='utf-8')
            logger.debug(f"✅ [ДИАГНОСТИКА] Запись чатов выполнена без ошибок")

            # ПРОВЕРКА РЕЗУЛЬТАТА
            logger.debug(f"🔍 [ДИАГНОСТИКА] Проверяем результат записи чатов...")
            ...
            logger.debug(f"✅ [ДИАГНОСТИКА] Файл чатов создан, размер: {file_size} байт")
            ...
            try:
                ...
                logger.debug(f"✅ [ДИАГНОСТИКА] Проверка чтения чатов:")
                logger.debug(f"   📊 Строк в файле: {len(saved_lines)}")
                logger.debug(f"   📊 Ожидалось строк: {len(chats_list)}")

                if saved_lines:
                    logger.debug(f"   💬 Первые 3 строки: {saved_lines[:3]}")

                # Сравниваем количество
                if len(saved_lines) == len(chats_list):
                    logger.debug(f"🎉 [ДИАГНОСТИКА] Количество записей чатов совпадает!")
                    return True
                else:
